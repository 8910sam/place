...
include("config.php");
  ...
  $fin=$con->query("SELECT * FROM sale  INNER JOIN customer ON customer.id WHERE sal_id='$_GET[sal_id]'")or die(mysqli_error());
...
<input type="text" name="quantity" class="form-control" value="<?php echo $ro["quantity"] ?>">
</div>
</div>
<div class="col-12 col-sm-6">
<div class="form-group">
<label>Price</label>
<input type="text" name="price" class="form-control" value="<?php echo $ro["pprice"] ?>" required>
</div>
</div>
</div>
<div class="col-12">
  <div class="form-group">
<button type="submit" name="save" class="btn btn-primary">Update Sale Records</button>
</div>
</div>
</form>
</div>
...
if(isset($_POST['save'])){
  ...
  $d=$_POST['price'];
  $tt=$d*$c;
  $in=$con->query("UPDATE sale SET id='$a',product_name='$b',quantity='$c',pprice='$d',ttprice='$tt' WHERE sal_id='$_GET[sal_id]'")or die(mysqli_error());
  ...
  if ($c>$q['quantity']) {
    ...
    $up=$con->query($u);
    echo"<script>alert('successful');window.location.replace('sale.php')</script>";
  }
  
  
  
  else{
    echo"<script>alert('Something wrong');window.location.replace('add-sale.php')</script>";
